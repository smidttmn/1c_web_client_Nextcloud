...
# Версия: 10.0.1 - УНИВЕРСАЛЬНАЯ НАСТРОЙКА ПРОКСИ (SSL + NON_SSL)
# ============================================================================
#
# ВАЖНО: Этот скрипт:
...
install_app() {
    ...
    if [ -f "$app_dest/appinfo/info.xml" ] && [ -f "$app_dest/lib/AppInfo/Application.php" ]; then
        print_success "Файлы приложения на месте"
    else
    ...
    # Примечание
    print_info "Приложение распаковано. Включите его через админ-панель Nextcloud:"
    print_info "  https://cloud.smidt.keenetic.pro/index.php/settings/apps"
}
