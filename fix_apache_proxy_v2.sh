#!/bin/bash
# Скрипт добавления ProxyPass для 1С в Apache конфиг (ПРАВИЛЬНАЯ ВЕРСИЯ)
...
# Находим строку с SSLCertificateKeyFile (после неё добавим)
LINE_NUM=$(grep -n "SSLCertificateKeyFile" "$APACHE_CONFIG" | head -1 | cut -d: -f1)

if [ -z "$LINE_NUM" ]; then
    echo "✗ Не найдено SSLCertificateKeyFile"
    exit 1
fi

# Вставляем ProxyPass ПОСЛЕ SSLCertificateKeyFile
...
# Вставляем после строки с SSLCertificateKeyFile
LINE_AFTER=$((LINE_NUM + 1))
sed -i "${LINE_AFTER}r /tmp/proxy_insert.txt" "$APACHE_CONFIG"
